...
repetida(Persona, Figurita):-
    persona(Persona),
    figurita(Figurita),
    findall(Figura, (tiene(Persona, Figura), Figura = Figurita), Figuras),
    length(Figuras, Cantidad),
    Cantidad > 1.
...
calculoInteres(Persona, Recibidas, NivelInteres):-
    ...
    NivelInteres is SumaAtractivos + Bonus.
...
validarCanjeFiguritas(Persona, FiguritasCanje):-
    findall(FiguritasDePersona, tiene(Persona, FiguritasDePersona), TodasFiguritasDePersona),
    msort(TodasFiguritasDePersona, FiguritasDePersonaOrdenadas),
...
% Generador de todos los subconjuntos de una lista. Partes de un Conjunto
subconjunto([], []).
...
universoRecibibles(OtraPersona, Recibidas):-
    findall(Figurita, (tiene(OtraPersona, Figurita)), ListaFiguritas),
    list_to_set(ListaFiguritas, Recibidas).
...
universoRepetidas(Persona, Dadas):-
    ...
    list_to_set(ListaFiguritas, Dadas).
